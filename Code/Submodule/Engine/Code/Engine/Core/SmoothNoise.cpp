...
#include "Engine/Math/MathUtilities.hpp"	// for SmoothStep3(); see "SmoothStep" on Wikipedia
#include "Engine/Math/Vector2.hpp"			// for Vector2( float x,y ) class/struct
#include "Engine/Math/Vector3.hpp"			// for Vector3( float x,y,z ) class/struct
#include "Engine/Math/Vector4.hpp"			// for Vector4( float x,y,z,w ) class/struct
...
{
	const float OCTAVE_OFFSET = 0.636764989593174f; // Translation/bias to add to each octave
	constexpr Vector2 gradients[ 8 ] = // Normalized unit vectors in 8 quarter-cardinal directions
	{
...
float Compute3dPerlinNoise( float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	constexpr Vector3 gradients[ 8 ] = // Traditional "12 edges" requires modulus and isn't any better.
	{
...
float Compute4dPerlinNoise( float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	constexpr Vector4 gradients[ 16 ] = // Hard to tell if this is any better in 4D than just having 8
	{
