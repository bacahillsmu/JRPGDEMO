...
#include "Engine/Math/Vec2.hpp"			// for Vector2( float x,y ) class/struct
#include "Engine/Math/Vec3.hpp"			// for Vector3( float x,y,z ) class/struct
#include "Engine/Math/Vec4.hpp"			// for Vector4( float x,y,z,w ) class/struct
...
{
	const float OCTAVE_OFFSET = 0.636764989593174f; // Translation/bias to add to each octave
	Vec2 gradients[ 8 ] = // Normalized unit vectors in 8 quarter-cardinal directions
	{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotSouthWest = DotProductVec2( gradientSW, displacementFromSW );
		float dotSouthEast = DotProductVec2( gradientSE, displacementFromSE );
		float dotNorthWest = DotProductVec2( gradientNW, displacementFromNW );
		float dotNorthEast = DotProductVec2( gradientNE, displacementFromNE );
...
float Compute3dPerlinNoise( float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotBelowSW = DotProductVec3( gradientBelowSW, displacementFromBelowSW );
		float dotBelowSE = DotProductVec3( gradientBelowSE, displacementFromBelowSE );
		float dotBelowNW = DotProductVec3( gradientBelowNW, displacementFromBelowNW );
		float dotBelowNE = DotProductVec3( gradientBelowNE, displacementFromBelowNE );
		float dotAboveSW = DotProductVec3( gradientAboveSW, displacementFromAboveSW );
		float dotAboveSE = DotProductVec3( gradientAboveSE, displacementFromAboveSE );
		float dotAboveNW = DotProductVec3( gradientAboveNW, displacementFromAboveNW );
		float dotAboveNE = DotProductVec3( gradientAboveNE, displacementFromAboveNE );
...
float Compute4dPerlinNoise( float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	Vec4 gradients[ 16 ] = // Hard to tell if this is any better in 4D than just having 8
	{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotBeforeBSW = DotProductVec4( gradientBeforeBSW, displacementFromBeforeBSW );
		float dotBeforeBSE = DotProductVec4( gradientBeforeBSE, displacementFromBeforeBSE );
		float dotBeforeBNW = DotProductVec4( gradientBeforeBNW, displacementFromBeforeBNW );
		float dotBeforeBNE = DotProductVec4( gradientBeforeBNE, displacementFromBeforeBNE );
		float dotBeforeASW = DotProductVec4( gradientBeforeASW, displacementFromBeforeASW );
		float dotBeforeASE = DotProductVec4( gradientBeforeASE, displacementFromBeforeASE );
		float dotBeforeANW = DotProductVec4( gradientBeforeANW, displacementFromBeforeANW );
		float dotBeforeANE = DotProductVec4( gradientBeforeANE, displacementFromBeforeANE );
		float dotAfterBSW = DotProductVec4( gradientAfterBSW, displacementFromAfterBSW );
		float dotAfterBSE = DotProductVec4( gradientAfterBSE, displacementFromAfterBSE );
		float dotAfterBNW = DotProductVec4( gradientAfterBNW, displacementFromAfterBNW );
		float dotAfterBNE = DotProductVec4( gradientAfterBNE, displacementFromAfterBNE );
		float dotAfterASW = DotProductVec4( gradientAfterASW, displacementFromAfterASW );
		float dotAfterASE = DotProductVec4( gradientAfterASE, displacementFromAfterASE );
		float dotAfterANW = DotProductVec4( gradientAfterANW, displacementFromAfterANW );
		float dotAfterANE = DotProductVec4( gradientAfterANE, displacementFromAfterANE );
